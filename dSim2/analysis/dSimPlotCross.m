...
figure; axis;
set(gca,'FontSize',14);
xlab = 'Proportion of Crossing Fibers';
lt = 'k-';
symb = {'o','s','d','v','p','x','^','+','h','*','>'};
for(ii=1:numel(uSpace))
   ...
   subplot(2,3,1);
   hold on;
   plot(crossFraction(inds),fa(inds),[lt symb{ii}]);
   hold off;
   subplot(2,3,2);
   hold on;
   plot(crossFraction(inds),md(inds),[lt symb{ii}]);
   hold off;
   subplot(2,3,4);
   hold on;
   plot(crossFraction(inds),l1(inds),[lt symb{ii}]);
   hold off;
   subplot(2,3,5);
   hold on;
   plot(crossFraction(inds),l2(inds),[lt symb{ii}]);
   hold off;
   subplot(2,3,6);
   hold on;
   plot(crossFraction(inds),l3(inds),[lt symb{ii}]);
   hold off;
...
outName = fullfile(dSimDir,'doc','figs','cross');
