...
public:
  ...
  int Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], DATATYPE interSectArray[]);
  ...
  int CountAllRects();													// Test
  int GetHeight();													// Test
  uint GetTreeArraySize();												// Test
  void CountNodesAndBranches(Node* a_node, uint* numNodes, uint* numBranches);						// Test
  void CreateIndexArray_old(uint* indexArray, uint indexArraySize, uint &treeArraySize);				// Test
  void CreateIndexArray();												// Test
  void RTree2Array_old(uint* indexArray, float* treeArray);								// Test
  void RTree2Array(float* treeArray, uint offset);									// Test
  void Node2Array(Node* a_node, uint* indexVec, float* RTreeArray);					// Test
...
protected:
 ...
 // bool Search(Node* a_node, Rect* a_rect, int& a_foundCount, bool /*__cdecl*/ a_resultCallback(DATATYPE a_data, void* a_context), void* a_context);
  bool Search(Node* a_node, Rect* a_rect, int& a_foundCount, DATATYPE interSectArray[]);
  void RemoveAllRec(Node* a_node);
  void Reset();
  void CountRec(Node* a_node, int& a_count);
  void CountChildRects(Node* a_node, int& a_count);
...
int RTREE_QUAL::Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], DATATYPE interSectArray[])
{
  ...
  //Search(m_root, &rect, foundCount, a_resultCallback, a_context);
	Search(m_root, &rect, foundCount, interSectArray);
...
typename RTREE_QUAL::Rect RTREE_QUAL::CombineRect(Rect* a_rectA, Rect* a_rectB)
{
  ...
  for(int index = 0; index < NUMDIMS; ++index)
  {
	...
	if (a_rectA->m_min[index] < a_rectB->m_min[index]){
		newRect.m_min[index] = a_rectA->m_min[index];
	} else{
		newRect.m_min[index] = a_rectB->m_min[index];
	}

	if (a_rectA->m_max[index] > a_rectB->m_max[index]){
		newRect.m_max[index] = a_rectA->m_max[index];
	} else{
		newRect.m_max[index] = a_rectB->m_max[index];
	}
...
bool RTREE_QUAL::Search(Node* a_node, Rect* a_rect, int& a_foundCount, DATATYPE interSectArray[])
{
  ASSERT(a_node);
  ASSERT(a_node->m_level >= 0);
  ASSERT(a_rect);
  ...
  if(a_node->IsInternalNode()) // This is an internal node in the tree
  {
	...
	if(!Search(a_node->m_branch[index].m_child, a_rect, a_foundCount, interSectArray))
        {
  ...
  else // This is a leaf node
  {
	...
	interSectArray[a_foundCount] = id;	
...
RTREE_TEMPLATE
int RTREE_QUAL::CountAllRects()
{
  int count = 0;
  CountChildRects(m_root, count);
...
RTREE_TEMPLATE
void RTREE_QUAL::CountChildRects(Node* a_node, int& a_count)
{
	a_count += a_node->m_count;
	if(a_node->IsInternalNode())  // not a leaf node
	{
		for(int index = 0; index < a_node->m_count; ++index)
		{
			CountChildRects(a_node->m_branch[index].m_child, a_count);
		}
	}
}

RTREE_TEMPLATE
int RTREE_QUAL::GetHeight()
{
	return m_root->m_level;
}


RTREE_TEMPLATE
void RTREE_QUAL::CountNodesAndBranches(Node* a_node, uint* numNodes, uint* numBranches){
		numNodes[a_node->m_level] += 1;
		numBranches[a_node->m_level] += a_node->m_count;
		if (a_node->m_level > 0){
			for (int n=0; n<a_node->m_count; n++){
				CountNodesAndBranches(a_node->m_branch[n].m_child, numNodes, numBranches);
			}	
		}
}
...
RTREE_TEMPLATE
uint RTREE_QUAL::GetTreeArraySize()
{
	return m_treeArraySize;
}


RTREE_TEMPLATE
void RTREE_QUAL::CreateIndexArray_old(uint* indexArray, uint indexArraySize, uint &treeArraySize){
	uint* numNodes = new uint[indexArraySize];
	memset(numNodes,0,(indexArraySize)*sizeof(uint));
	uint* numBranches = new uint[indexArraySize];
	memset(numBranches,0,(indexArraySize)*sizeof(uint));

	CountNodesAndBranches(m_root,numNodes,numBranches);

	//printf("(in RTree.h): Number of nodes in level 3: %u\n", numNodes[3]);
	//printf("(in RTree.h): Number of branches in level 3: %u\n", numBranches[3]);
	//printf("(in RTree.h): Number of nodes in level 2: %u\n", numNodes[2]);
	//printf("(in RTree.h): Number of branches in level 2: %u\n", numBranches[2]);
	//printf("(in RTree.h): Number of nodes in level 1: %u\n", numNodes[1]);
	//printf("(in RTree.h): Number of branches in level 1: %u\n", numBranches[1]);
	//printf("(in RTree.h): Number of nodes in level 0: %u\n", numNodes[0]);
	//printf("(in RTree.h): Number of branches in level 0: %u\n", numBranches[0]);

	for (int n=indexArraySize-1; n>=0; n--){
		printf("(in RTree.h::CreateIndexArray): Number of nodes in level %u: %u\n", n, numNodes[n]);
		printf("(in RTree.h::CreateIndexArray): Number of branches in level %u: %u\n", n, numBranches[n]);
	}

	//indexArray = new uint[indexArraySize];
	//memset(indexArray,0,(indexArraySize)*sizeof(uint));
	uint nodeSize = 2;
	uint branchSize = 7;
	indexArray[indexArraySize-1] = 0;
	printf("(in RTree.h::CreateIndexArray): indexArray[%u] (before R-Tree construction): %u\n", indexArraySize-1, indexArray[indexArraySize-1]);
	for (int m = indexArraySize-2; m >= 0; m--){
		indexArray[m] = indexArray[m+1] + numNodes[m+1]*nodeSize + numBranches[m+1]*branchSize;
		printf("(in RTree.h::CreateIndexArray): indexArray[%u] (before R-Tree construction): %u\n", m, indexArray[m]);
	}

	//printf("(in RTree.h): indexArray[3] (before R-Tree construction): %u\n", indexArray[3]);
	//printf("(in RTree.h): indexArray[2] (before R-Tree construction): %u\n", indexArray[2]);
	//printf("(in RTree.h): indexArray[1] (before R-Tree construction): %u\n", indexArray[1]);
	//printf("(in RTree.h): indexArray[0] (before R-Tree construction): %u\n", indexArray[0]);

	treeArraySize = indexArray[0] + numNodes[0]*nodeSize + numBranches[0]*branchSize;
	m_treeArraySize = treeArraySize;
}


RTREE_TEMPLATE
void RTREE_QUAL::CreateIndexArray(){
	
	m_indexArraySize = GetHeight() + 1;
	uint* numNodes = new uint[m_indexArraySize];
	memset(numNodes,0,(m_indexArraySize)*sizeof(uint));
	uint* numBranches = new uint[m_indexArraySize];
	memset(numBranches,0,(m_indexArraySize)*sizeof(uint));
	m_indexArray = new uint[m_indexArraySize];
	memset(m_indexArray,0,m_indexArraySize*sizeof(uint));

	CountNodesAndBranches(m_root,numNodes,numBranches);

	/*for (int n=m_indexArraySize-1; n>=0; n--){
		printf("(in RTree.h::CreateIndexArray): Number of nodes in level %u: %u\n", n, numNodes[n]);
		printf("(in RTree.h::CreateIndexArray): Number of branches in level %u: %u\n", n, numBranches[n]);
	}*/
	...
	uint nodeSize = 2;
	uint branchSize = 7;
	m_indexArray[m_indexArraySize-1] = 0;
	//printf("(in RTree.h::CreateIndexArray): indexArray[%u] (before R-Tree construction): %u\n", m_indexArraySize-1, m_indexArray[m_indexArraySize-1]);
	for (int m = m_indexArraySize-2; m >= 0; m--){
		m_indexArray[m] = m_indexArray[m+1] + numNodes[m+1]*nodeSize + numBranches[m+1]*branchSize;
		//printf("(in RTree.h::CreateIndexArray): indexArray[%u] (before R-Tree construction): %u\n", m, m_indexArray[m]);
	}

	m_treeArraySize = m_indexArray[0] + numNodes[0]*nodeSize + numBranches[0]*branchSize;

	//printf("(in RTree.h::CreateIndexArray): m_treeArraySize: %u\n", m_treeArraySize);
}


RTREE_TEMPLATE
void RTREE_QUAL::RTree2Array_old(uint* indexArray, float* treeArray){

	Node2Array(m_root, indexArray, treeArray);

}


RTREE_TEMPLATE
void RTREE_QUAL::RTree2Array(float* treeArray, uint offset){

	uint *indexVec = new uint[m_indexArraySize];
	
	//printf("(in RTree.h::RTree2Array): m_indexArraySize: %u\n", m_indexArraySize);
	for (uint k=0; k<m_indexArraySize; k++){
		//printf("(in RTree.h::RTree2Array): m_indexArray[%u]: %u\n", k, m_indexArray[k]);
		indexVec[k] = m_indexArray[k] + offset;
	}

	Node2Array(m_root, indexVec, treeArray);

}


RTREE_TEMPLATE
void RTREE_QUAL::Node2Array(Node* a_node, uint* indexVec, float* RTreeArray){
	uint current_level = a_node->m_level;
	//printf("current_level: %u\n", current_level);
	//printf("indexVec[%u]: %u\n", current_level, indexVec[current_level]);
	//printf("RTreeArray[indexVec[%u]]: %g\n", current_level, RTreeArray[indexVec[current_level]]);
	//printf("Assigning new value to RTreeArray[indexVec[%u]]\n",current_level);
	RTreeArray[indexVec[current_level]] = current_level;					// Write the level of the node (0 for leaves, treeHeight-1 for top node)
	//printf("RTreeArray[indexVec[%u]]: %g\n", current_level, RTreeArray[indexVec[current_level]]);
	//printf("Incrementing index\n");
	indexVec[current_level] += 1;
	//printf("indexVec[%u]: %u\n", current_level, indexVec[current_level]);
	//printf("RTreeArray[indexVec[%u]]: %g\n", current_level, RTreeArray[indexVec[current_level]]);
	//printf("Assigning new value to RTreeArray[indexVec[%u]]\n",current_level);
	RTreeArray[indexVec[current_level]] = a_node->m_count;
	//printf("RTreeArray[indexVec[%u]]: %g\n", current_level, RTreeArray[indexVec[current_level]]);
	//printf("Incrementing index\n");
	indexVec[current_level] += 1;

	for (uint n=0; n<a_node->m_count; n++){
		if (current_level>0){
			RTreeArray[indexVec[current_level]] = indexVec[current_level - 1];
			Node2Array(a_node->m_branch[n].m_child, indexVec, RTreeArray);
		} else {
			RTreeArray[indexVec[current_level]] = a_node->m_branch[n].m_data;
		}
		indexVec[current_level] += 1;
		RTreeArray[indexVec[current_level]] = a_node->m_branch[n].m_rect.m_min[0];
		indexVec[current_level] += 1;
		RTreeArray[indexVec[current_level]] = a_node->m_branch[n].m_rect.m_min[1];
		indexVec[current_level] += 1;
		RTreeArray[indexVec[current_level]] = a_node->m_branch[n].m_rect.m_min[2];
		indexVec[current_level] += 1;
		RTreeArray[indexVec[current_level]] = a_node->m_branch[n].m_rect.m_max[0];
		indexVec[current_level] += 1;
		RTreeArray[indexVec[current_level]] = a_node->m_branch[n].m_rect.m_max[1];
		indexVec[current_level] += 1;
		RTreeArray[indexVec[current_level]] = a_node->m_branch[n].m_rect.m_max[2];
		indexVec[current_level] += 1;
	}
}
