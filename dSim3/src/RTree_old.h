...
public:
  ...
  int GetHeight();													// Test
  int GetTreeArraySize();												// Test
  void CountNodesAndBranches(Node* a_node, uint* numNodes, uint* numBranches);						// Test
  void CreateIndexArray(uint* indexArray, uint indexArraySize, uint &treeArraySize);			// Test
  void RTree2Array(uint* indexArray, float* treeArray);									// Test
  void Node2Array(Node* a_node, uint* indexVec, float* RTreeArray);							// Test
  void CreateIndexArray(float* testArray, uint* indexArray, uint indexArraySize, uint &treeArraySize);			// Test
...
RTREE_TEMPLATE
int RTREE_QUAL::GetTreeArraySize()
{
...
RTREE_TEMPLATE
void RTREE_QUAL::CreateIndexArray(uint* indexArray, uint indexArraySize, uint &treeArraySize){
	...
	for (int n=indexArraySize-1; n>=0; n--){
		printf("(in RTree.h): Number of nodes in level %u: %u\n", n, numNodes[n]);
		printf("(in RTree.h): Number of branches in level %u: %u\n", n, numBranches[n]);
	}
	...
	printf("(in RTree.h): indexArray[%u] (before R-Tree construction): %u\n", indexArraySize-1, indexArray[indexArraySize-1]);
...
RTREE_TEMPLATE
void RTREE_QUAL::RTree2Array(uint* indexArray, float* treeArray){
...
void RTREE_QUAL::Node2Array(Node* a_node, uint* indexVec, float* RTreeArray){
	...
	RTreeArray[indexVec[current_level]] = current_level;
	//printf("RTreeArray[indexVec[%u]]: %g\n", current_level, RTreeArray[indexVec[current_level]]);
